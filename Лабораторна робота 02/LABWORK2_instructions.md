...
| **Тест 6** | Виняткова ситуація (введення нечислових даних) | `A = a`, `B = b`, `C = c`, `K = d` | Повідомлення про помилку:<br>`Помилка введення: введено неціле значення!` | Перевірка системи валідації вхідних параметрів та запобігання аварійному завершенню роботи програми. |
| **Тест 7** | Виняткова ситуація (введення нечислових даних) | `A = 0xA5`, `B = 0xB6`, `C = 0xC7`, `K = 0xD8` | Програма видала повідомлення про помилку:<br>`Помилка введення: введено неціле значення!`<br>проте вона успішно зчитала перший `0` з введеного значення `0xA5` у змінну `A` і достроково завершила роботу при спробі зчитати `xA5` до змінної `B` | Перевірка системи валідації вхідних параметрів та запобігання аварійному завершенню роботи програми. |

> *Примітка*: треба враховувати особливості роботи оператора `cin`, наприклад можлива така ситуація:
>
> ```cmd
> ========================================================
>  Лабораторна робота №2. Приклад розв'язання задачі
...
> ========================================================
> 
> Введіть нижню межу інтервалу (A): 1 20 18 3
> Введіть верхню межу інтервалу (B): Введіть граничне значення (C): Введіть дільник (K, не 0): 
> --- Результати обробки числових даних ---
> Числа з [1, 20], які менші за 18 та кратні 3 (у зворотному порядку):
> 15 12 9 6 3 
> 
> Усього знайдено чисел: 5
> ========================================================
> ```
>
> Програма одразу сприйняла всі числа введені через пробіл і провела розрахунок.
